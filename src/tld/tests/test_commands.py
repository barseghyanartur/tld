...
from ..commands import update_tld_names
...
class TestCommands(unittest.TestCase):
    ...
    @log_info
    def test_1_update_tld_names_module(self):
        """Test updating the tld names (re-fetch mozilla source)."""
        res = subprocess.check_output(shlex.split('python src/tld/commands/update_tld_names.py')).strip()
        self.assertEqual(res, b'True')
        return res
